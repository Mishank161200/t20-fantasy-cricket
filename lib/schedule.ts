...
// - Match date < current date (past) = completed
// - Match date == current date (same day) = live
// - Match date > current date (future) = scheduled
function getMatchStatus(matchDate: Date): 'completed' | 'live' | 'scheduled' {
  const now = new Date();
  const match = new Date(matchDate);

  // Get the date portion only (ignore time)
  const nowDate = new Date(now.getFullYear(), now.getMonth(), now.getDate());
  const matchDateOnly = new Date(match.getFullYear(), match.getMonth(), match.getDate());

  if (matchDateOnly < nowDate) {
    return 'completed';
  } else if (matchDateOnly.getTime() === nowDate.getTime()) {
    return 'live';
  } else {
    return 'scheduled';
  }
...
const BASE_SCHEDULE: Omit<Match, 'status'>[] = [
  // GROUP STAGE - Matches 1-40 (Feb 7-20, 2026)
